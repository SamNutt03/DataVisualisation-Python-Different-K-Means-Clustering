...
def initialSelection(data, k):
    #Chooses initial centroid randomly, then probabilistically chooses the other centroids, farther away points are more likely to be selected
    #Input: data from arrays, number of clusters k
    #Output: list of k randomly selected points from the data

    centroids = [random.choice(data.tolist())] #first pick a random data point
    for _ in range(1, k): #loop through for the number of centroids
        distances = [min([ComputeDistance(point, np.array(c)) for c in centroids]) for point in data] ##calculate the euclidian distances from each point to the centroids
        probabilities = distances / np.sum(distances) #normalises the distances to make a probability distribution for picking 
        chosen_index = np.random.choice(len(data), p=probabilities) # pick the new centroid index with the probability distribution (further ones are higher probability)
        centroids.append(data[chosen_index].tolist()) #add that chosen centroid to the list of centroids
    return centroids
...
def plot_silhouettee(scores):
    ...
    plt.grid(True)
    plt.savefig('KMeansplusplus-Plot.png')  #Saves the plot to a png file in folder
    plt.show()

def main():
    #Main function to run k-means++ clustering algorithm
    
    # loading dataset in from the file with function
    _, data = load_dataset()
    scores = []
